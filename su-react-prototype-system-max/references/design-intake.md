...
- [Evidence Access Level](#evidence-access-level)
...
Record one level before implementation or review:

| Level | Evidence | Claim boundary |
| --- | --- | --- |
| `complete` | Scoped nodes, meaningful names, variables/styles, relevant assets, and screenshots | Claim only the scoped naming, token, asset, and visual evidence inspected |
| `partial` | Frames are visible but variables, bindings, variants, or assets are incomplete | List missing categories and do not claim complete coverage |
| `screenshot-only` | A rasterized design without authoritative metadata | Treat names, values, breakpoints, and tokens as visual inference |
| `unavailable` | The named source cannot be inspected | Use repository evidence only and report design verification as `NOT RUN` |

Record browser evidence separately. Complete Figma access does not prove rendered fidelity, and a working browser does not recover unavailable source metadata.

If access improves during the task, update the level and re-check decisions that depended on inference. Never promote an inferred value to source-backed evidence without inspecting its authoritative source.
...
3. Record meaningful component, component-set, variant, property, and internal layer names.
4. Identify missing, generic, or autogenerated names that require contextual inference.
5. Inspect variables, collections, modes, shared styles, library components, layout constraints, and prototype links to determine whether the file contains a design system.
6. Export or reuse assets through the design source when possible, and verify persisted asset formats from their content rather than URL suffixes.
7. Inspect sibling frames only when they clarify a state, breakpoint, or transition needed by the task.

When any item is unavailable, keep the evidence level at `partial` and record the missing category. Do not substitute visual similarity for authoritative variable, binding, or component-property evidence.
...
Build an asset map with source, declared MIME, content-detected format, final filename, usage, import strategy, and crop behavior. Prefer:
...
When the Figma integration provides a stable image or SVG source, including a localhost source intended for direct use, preserve it as instructed. If the asset is copied into the repository, treat the URL suffix and suggested filename as untrusted: inspect the downloaded bytes or parseable SVG XML, apply the matching suffix, and audit it before adding imports.
...
- What is the evidence access level, and which claims does it permit?
- Which local components and tokens can be reused?
- Which Figma names can be preserved, and which names require contextual inference?
- Does the Figma file contain a design system that must be mapped into Tailwind first?
- What are the component, screen, and flow boundaries?
- Which states must be reachable?
- Which assets are authoritative?
- Do persisted asset contents, suffixes, import strategies, and references agree?
